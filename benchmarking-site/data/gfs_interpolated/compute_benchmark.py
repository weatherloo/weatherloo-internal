#!/usr/bin/env python3
"""Compute GFS interpolated-at-station benchmark JSON for issue #8.

Fetches NOAA GFS 00/06/12/18Z (0.25°) from AWS Open Data, bilinearly
...
def legacy_init_filename(init_dt: datetime) -> str | None:
    """Pre-multi-cycle naming (00Z only): ``2025-MM-DD.json``."""
    if init_dt.hour == 0:
        return f"{init_dt.strftime('%Y-%m-%d')}.json"
    return None
...
def process_init(
    ...
    if resume and out_path.exists():
        return fname
    legacy = legacy_init_filename(init_dt)
    if resume and legacy and (out_dir / legacy).exists():
        return fname
