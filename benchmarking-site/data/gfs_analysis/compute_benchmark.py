#!/usr/bin/env python3
"""Compute GFS analysis benchmark JSON for issue #9.

Fetches NOAA GFS 00/06/12/18Z (0.25°) analysis field (f000) from AWS Open
Data, bilinearly interpolates 2 m temperature and 10 m wind (from u/v
...
Also writes a consolidated ``gfs_analysis_<year>.npz`` (see
...
def init_json_paths(out_dir: Path, year: int) -> list[Path]:
    """Per-init JSON files for the current naming scheme (excludes legacy daily)."""
...
def build_init_json(
    ...
    for station_id, coords in STATIONS.items():
        # Fetch f000 once per station — this single analysis value is reused
        # as the forecast for every lead time.
        fcst_t2m, fcst_wind = fetch_gfs_point(
            date_str, init_dt.hour, 0, coords["lat"], coords["lon"]
        )
...
def write_metadata(out_dir: Path, year: int) -> None:
    meta = {
        ...
        "forecast_logic": "Single f000 analysis value at init reused for all 12 lead times; obs verified at init + lead_hours",
