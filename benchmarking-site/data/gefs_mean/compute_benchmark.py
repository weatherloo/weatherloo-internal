#!/usr/bin/env python3
"""Compute GEFS ensemble-mean benchmark JSON.

Fetches NOAA GEFS 00/06/12/18Z ensemble mean (``geavg``) at 0.5° from AWS Open
Data (``noaa-gefs-pds``), bilinearly interpolates 2 m temperature and 10 m wind
(from u/v components) to each station, and writes per-initialization metric
files in this directory (``data/gefs_mean/``).

The AWS archive provides pre-averaged ``geavg`` GRIB2 files (21-member mean on
the grid); we treat that field as a single deterministic forecast per lead time.

Also writes a consolidated ``gefs_mean_<year>.npz`` (see
...
CACHE_DIR = REPO_ROOT / ".cache" / "gefs_grib"
...
def gefs_grib_url(date_yyyymmdd: str, cycle_hour: int, fxx: int) -> str:
    cycle = f"{cycle_hour:02d}"
    cycle_tag = f"{cycle_hour:02d}z"
    return (
        f"{AWS_BASE}/gefs.{date_yyyymmdd}/{cycle}/atmos/pgrb2ap5/"
        f"geavg.t{cycle_tag}.pgrb2a.0p50.f{fxx:03d}"
    )


def fetch_gefs_mean_point(
    date_yyyymmdd: str,
    cycle_hour: int,
    fxx: int,
    lat: float,
    ...
    """Return (t2m degC, wind_speed km/h) from GEFS ensemble mean at a point."""
    cycle_tag = f"{cycle_hour:02d}z"
    grib_url = gefs_grib_url(date_yyyymmdd, cycle_hour, fxx)
    cache_key = f"{date_yyyymmdd}_{cycle_tag}_geavg_f{fxx:03d}"
...
def build_init_json(
    ...
    for station_id, coords in STATIONS.items():
        ...
        for lead in LEAD_TIMES:
            ...
            fcst_t2m, fcst_wind = fetch_gefs_mean_point(
...
def write_metadata(out_dir: Path, year: int) -> None:
    meta = {
        "method_id": METHOD_ID,
        "model": "NOAA GEFS ensemble mean (geavg)",
        "cycles": ["00Z", "06Z", "12Z", "18Z"],
        "grid": "0p50",
        "ensemble": "21-member mean on grid (gec00 + gep01–gep20), pre-averaged in AWS archive",
        "year": year,
        ...
        "source": "https://github.com/awslabs/open-data-docs/tree/main/docs/noaa/noaa-gefs-pds",
...
def main() -> None:
    ...
    parser.add_argument(
        "--export-npz-only",
        action="store_true",
        help="Rebuild NPZ from existing JSON files without fetching GEFS",
    )
